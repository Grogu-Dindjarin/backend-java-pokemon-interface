...
public class ElectricPokemon extends Pokemon {
    ...
    void thunderPunch(Pokemon pokemon, Pokemon enemy) {
        ...
        int pokemonEnemyHp = enemy.setHp(enemy.getHp() - thunderPunchDmg);
        System.out.printf("%s! Crackle! Zap! The Thunder Punch electrifies the air with a sharp crack and a surge of power! dealing -%sdmg , %s has %shp left %n",
                pokemon.getSound(), Main.ANSI_RED + thunderPunchDmg + Main.ANSI_RESET, enemy.getName(), Main.ANSI_BLACK + pokemonEnemyHp + Main.ANSI_RESET);
    }
    ...
    void electroBall(Pokemon pokemon, Pokemon enemy) {
        ...
        int pokemonEnemyHp = enemy.setHp(enemy.getHp() - electroBallDmg);
        System.out.printf("%s! %s unleashes a crackling sphere of electrical energy, zapping %s with a surge of electrifying power.! dealing -%sdmg , %s has %shp left %n",
                pokemon.getSound(), pokemon.getName(), enemy.getName(), Main.ANSI_RED + electroBallDmg + Main.ANSI_RESET, enemy.getName(), Main.ANSI_BLACK + pokemonEnemyHp + Main.ANSI_RESET);
    }
    ...
    void thunder(Pokemon pokemon, Pokemon enemy) {
        if (enemy.getType().equals(pokemon.getType())) {
            ...
            System.out.printf("Fzzzz! CRACK! BOOM! %s is surprised to see its Thunder move heal %s with %shp, %s! %s has %shp left !%n",
                    pokemon.getSound(), enemy.getName(), Main.ANSI_GREEN + hpBoost + Main.ANSI_RESET, enemy.getSound(), enemy.getName(), Main.ANSI_BLACK + pokemonEnemyHp + Main.ANSI_RESET);
        } else {
            int baseDmg = 15;
            int thunderDmg = calculateDamage(baseDmg, getOpponentMultiplier(enemy, typeAttackMultipliers));

            int pokemonEnemyHp = enemy.setHp(enemy.getHp() - thunderDmg);
            System.out.printf("%s! %s releases a thunderous bolt, crackling with electric energy as it strikes %s! Fzzzz! CRACK! BOOM! dealing -%sdmg , %s has %shp left %n",
                    pokemon.getSound(), pokemon.getName(), enemy.getName(), Main.ANSI_RED + thunderDmg + Main.ANSI_RESET, enemy.getName(), Main.ANSI_BLACK + pokemonEnemyHp + Main.ANSI_RESET);
        }
    }
    ...
    void voltTackle(Pokemon pokemon, Pokemon enemy) {
        ...
        int pokemonEnemyHp = enemy.setHp(enemy.getHp() - voltTackleDmg);
        System.out.printf("%s! A crackling surge of electricity as %s charges into %s! dealing -%sdmg , %s has %shp left %n",
                pokemon.getSound(), pokemon.getName(), enemy.getName(), Main.ANSI_RED + voltTackleDmg + Main.ANSI_RESET, enemy.getName(), Main.ANSI_BLACK + pokemonEnemyHp + Main.ANSI_RESET);
    }
}
